...
process ExtractSNP {
     ...
     output:
         ...
         file("${sample_id}.gene.tsv") into (resistome_hits)
...
process SNPconfirmation {
     ...
     output:
         set sample_id, file("*rgi_strict_hits.csv") into snp_long_hits

     """
     python $baseDir/bin/RGI_aro_hits.py ${rgi}
     """
}
...
process Confirmed_AMR_hits {
     ...
     input:
         set sample_id, file(rgi_counts) from snp_long_hits
         set file(megares_counts) from resistome_hits
         
     output:
         set sample_id, file("${sample_id}*confirmed_counts") into confirmed_counts

     """
     python $baseDir/bin/RGI_long_combine.py ${rgi_counts} ${megares_counts} ${sample_id}.SNP_confirmed_counts
     """
}


confirmed_counts.toSortedList().set { confirmed_amr_l_to_w }
...
process Confirmed_LongToWide {
     ...
     input:
         file(confirmed_resistomes) from confirmed_amr_l_to_w
                 
     output:
         file("SNP_confirmed_AMR_analytic_matrix.csv") into final_confirmed_matrix

     """

     mkdir ret
     python3 $baseDir/bin/amr_long_to_wide.py -i ${confirmed_resistomes} -o ret
     mv ret/AMR_analytic_matrix.csv SNP_confirmed_AMR_analytic_matrix.csv
     """
}
