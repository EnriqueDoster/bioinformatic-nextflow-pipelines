...
def rgi_output(rgi_file):
    ...
    with open(rgi_file, 'r') as rgifile:
        ...
        sample_name = header.index('ORF_ID')
        ...
        # Define a dictionary where ORF_names are keys and the items are the rest of the desired elements
        rgi_dict = {}

        # Go through the file and fill out the dictionary without repeats
        reader = csv.reader(rgifile, delimiter="\t")
        for row in reader:
            parsed_name = (row[sample_name].split('#'))[0]
            if parsed_name not in rgi_dict.keys():
                temp_holder = [row[category], row[best_hit_aro], row[aro], sum_hits, row[model_type]]
                rgi_dict[parsed_name] = temp_holder
                sum_hits += 1

        # Close the file
        rgifile.close()

        # Go through the dictionary update the sum_hits
        for value in rgi_dict.values():
            value[3] = sum_hits
    ...
    if perf_in_dict:
        with open(perf_file_name, 'w', newline='\n') as perf_file:
            ...
            perf_write.writerow(["ORF_ID", "Cut_Off", "Best_Hit_ARO", "ARO", "Sum_Hits", "Model_Type"])
            for perf_key, perf_item in rgi_dict.items():
                if perf_item[0] == "Perfect":
    ...
    if strict_in_dict:
        with open(strict_file_name, 'w', newline='\n') as strict_file:
            ...
            strict_write.writerow(["ORF_ID", "Cut_Off", "Best_Hit_ARO", "ARO", "Sum_Hits", "Model_Type"])
            for strict_key, strict_item in rgi_dict.items():
                if strict_item[0] == "Strict":
    ...
    if loose_in_dict:
        with open(loose_file_name, 'w', newline='\n') as loose_file:
            ...
            loose_write.writerow(["ORF_ID", "Cut_Off", "Best_Hit_ARO", "ARO", "Sum_Hits", "Model_Type"])
            for loose_key, loose_item in rgi_dict.items():
                if loose_item[0] == "Loose":
